...
# TODO pay attention to this when using for other test sets.
dimensions = [get_dimension(pb) for pb in manifold_benchmarks]
...
with_theme(theme_latexfonts()) do
    ...
    stairs!(0:k_max, daks[1, :]; label = L"\text{RDFO}", linewidth = 2, color = :green)
    # stairs!(0:k_max, daks[4, :]; label = L"\text{RDFO} ($\mathrm{Exp}$)", linewidth = 2, color = :red)
    stairs!(0:k_max, daks[2, :]; label = "MADS (Manopt.jl)", linewidth = 2, color = :blue)
    # stairs!(0:k_max, daks[1, :]; label = "Coordonnées sphériques", linewidth = 2, color = :goldenrod1)
    axislegend(position = :rb; labelsize = 25)
    CairoMakie.save("/home/sblelong/msc-thesis/thesis/figures/num-manopt-data-profile-en-$(τ).pdf", fig; px_per_unit = 4)
end
