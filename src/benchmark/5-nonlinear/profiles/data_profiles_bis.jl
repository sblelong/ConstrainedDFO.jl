...
for (alg_name, alg_data) in data_dict
    ...
    for id_instance in 1:n_instances
        ...
        data_alg_pb = JSON.parsefile(joinpath(base_data_path, "$(alg_name)", "$(id_instance)-8.json"))
...
for id_instance in 1:n_instances
    ...
    f0s[id_instance] = maximum(first_feasible_objectives)
end

# Find dimensions of all problems.
# TODO pay attention to this when using for other test sets.
dimensions = [get_dimension(pb) for pb in nonlinear_benchmarks]
...
for alg_id in 1:n_algs
    for id_instance in 1:n_instances
        ...
        Naps[alg_id, id_instance] = isnothing(first_solved_index) ? STUPID_MAX_NAPS : first_solved_index
    end
end
...
with_theme(theme_latexfonts()) do
    ...
    Axis(
        ...
        ylabel = L"Proportion of $\tau$-solved problems",
        xlabelsize = 22,
        ylabelsize = 22
    )
    stairs!(0:k_max, daks[2, :]; label = "RDFO", linewidth = 2, color = :green)
    # stairs!(0:k_max, daks[1, :]; label = "MADS-BP", linewidth = 2, color = :red)
    # stairs!(0:k_max, daks[4, :]; label = "MADS-PIP", linewidth = 2, color = :red)
    stairs!(0:k_max, daks[1, :]; label = "COBYLA", linewidth = 2, color = :blue)
    # stairs!(0:k_max, daks[1, :]; label = "MADS-BP", linewidth = 2, color = :green, linestyle = :dash)
    axislegend(position = :rb; labelsize = 25)
    CairoMakie.save("/home/sblelong/msc-thesis/thesis/figures/num-nonlinear-data-profile-en-$(τ).pdf", fig; px_per_unit = 4)
end
