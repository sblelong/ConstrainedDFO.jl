...
function plot_iterates(S::AbstractDFSolver, BP::AbstractBenchmarkProblem; invertibility_bound::AbstractInvertibilityBound = default_invertibility_bound(get_equality_manifold(BP), default_retraction_method(get_equality_manifold(BP))))
    ...
    return with_theme(theme_latexfonts()) do
        ...
        # text!(x0[1] - 0.14, x0[2] - 0.14; text = L"x_0", fontsize = 25, color = palette[2])

        # x1 = ih[2][1, :]
        # scatter!([x1[1]], [x1[2]]; marker = :circle, color = palette[3], markersize = 15, strokecolor = :black, strokewidth = 1)
        # text!(x1[1] - 0.13, x1[2] + 0.03; text = L"x_1", fontsize = 25, color = palette[3])

        # x2 = ih[3][1, :]
        # scatter!([x2[1]], [x2[2]]; marker = :circle, color = palette[4], markersize = 15, strokecolor = :black, strokewidth = 1)
        # text!(x2[1] - 0.05, x2[2] + 0.07; text = L"x_2", fontsize = 25, color = palette[4])
        ...
        text!(xrdir1[1] - 0.13, xrdir1[2] + 0.05; text = "(", color = :gray70, fontsize = 30, rotation = -π / 2, font = :bold)
        text!(xrdir2[1] + 0.13, xrdir2[2] - 0.05; text = "(", color = :gray70, fontsize = 30, rotation = π / 2, font = :bold)
        ...
        # scatter!([xi[1]], [xi[2]]; marker = :circle, color = it_color, markersize = 15, strokecolor = :black, strokewidth = 1)
        # text!(xi[1] - 0.25, xi[2] - 0.35; text = L"x_{%$(it-1)}", fontsize = 30, color = it_color)
        # scatter!(it_data[:, 1], it_data[:, 2]; marker = :circle, markersize = 10, color = it_color, strokewidth = 0)
        # ################################################
        ...
        # text!(xtdir1[1] - 0.05, xtdir1[2] - 0.25; text = L"\mathrm{T}_{x_1}\mathbb{S}^1", fontsize = 25, color = :gray70)
        # d = ds[it][2, :]
        # bpd = base_point .+ d
        # scatter!([bpd[1]], [bpd[2]]; marker = :circle, markersize = 10, color = it_color, strokewidth = 0)
        ...
        ##################### X2 ######################################
        # text!(xtdir1[1] - 0.5, xtdir1[2] - 0.2; text = L"\mathrm{T}_{x_2}\mathbb{S}^1", fontsize = 25, color = :gray70)
        # d = ds[it][2, :]
        # bpd = base_point .+ d
        # scatter!([bpd[1]], [bpd[2]]; marker = :circle, markersize = 10, color = it_color, strokewidth = 0)
        # lines!([base_point[1] + d[1], it_data[2, 1]], [base_point[2] + d[2], it_data[2, 2]]; linestyle = :dot, color = it_color)
        ...
        # text!(xtdir2[1] - 0.4, xtdir2[2] - 0.1; text = L"\mathrm{T}_{x_3}\mathbb{S}^1", fontsize = 25, color = :gray70)
        # d = ds[it]
        # bpd = base_point .+ transpose(d)
        # scatter!(bpd[1, :], bpd[2, :]; marker = :circle, markersize = 10, color = it_color, strokewidth = 0)
        # for i in 1:size(d)[1]
...
export plot_iterates, plot_iterates_rayleigh, plot_iterates_lin_exp
